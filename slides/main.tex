...
\end{frame}
%----------------------------------------------------------------------------------------------------------
\begin{frame}{Декодирование визуальной информации из сигналов мозга}
...
\begin{enumerate}
\justifying
    \item Учесть временную структуру фМРТ через анализ гемодинамической задержки.
...
\justifying
Пусть $\mathcal{X}^{(m)}$ обозначает пространство наблюдений модальности $m$, где $m = 1, \ldots, M$. 
Тогда совместное пространство нейробиологических данных:
\[
\mathcal{X} = \mathcal{X}^{(1)} \times \ldots \times \mathcal{X}^{(M)}.
\]
Визуальный стимул или его характеристика описываются элементом $\mathbf{y} \in \mathcal{Y}$.

Требуется построить отображение декодирования $\mathbf{g}$:
\[
...
\begin{block}{Частные случаи}
    \begin{itemize}
        \item Классификация временных рядов фМРТ:
        \[
        \mathbf{g}: \mathcal{X}_{f} \rightarrow \{1,\ldots,C\}.
        \]
        \item Реконструкция изображений по совместным фМРТ--ЭЭГ:
        \[
...
\end{frame}
%----------------------------------------------------------------------------------------------------------
\begin{frame}{Анализ гемодинамической задержки}
\justifying
...
\]
где $\mu$ и $\nu$~--- частоты дискретизации фМРТ и видеоряда соответственно. Гиперпараметр~$\Delta t$ обеспечивает согласование временной шкалы стимула и BOLD-отклика.

\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{figs/mse_dt.pdf}
\end{figure}
Зависимость MSE от $\Delta t$ имеет минимум; на локализованной зрительной коре он отчетливо выражен. Полученное значение $\Delta t \approx 5$~с согласуется с нейрофизиологическими оценками гемодинамической задержки.
\end{frame}
%----------------------------------------------------------------------------------------------------------
...
\begin{figure}
	\centering
	\includegraphics[width=0.98\linewidth]{figs/Figure_2.pdf}
\end{figure}
\vspace{-4mm}
...
Отображение $\mathbf{g}: \mathcal{X}_{f} \rightarrow \{1,\ldots, C\}$ представляется как $\mathbf{g} = \bm{\varphi} \circ \bm{\psi} \circ \mathcal{A}$
\begin{align*}
...
Отображение $\bm{\psi}$ представляется как конкатенация $\bm{\psi} = \bm{\psi}_1 \oplus \ldots \oplus \bm{\psi}_C$
\begin{equation*}
    \bm{\psi}_k: \mathbb{R}^{\tau \times X/k_s \times Y/k_s \times Z/k_s} \rightarrow \mathbb{R}^{d_k}, \quad \bm{\psi}_k = \bm{\pi}_k \circ \bm{f}_k, \quad d = \sum_{k=1}^C d_k, \quad d_k = \dfrac{h_k(h_k+1)}{2},
\end{equation*}
\begin{align*}
...
\end{frame}
%----------------------------------------------------------------------------------------------------------
\begin{frame}{Анализ масок активности мозга}
\justifying
\begin{itemize}
...
\end{frame}
%----------------------------------------------------------------------------------------------------------
\begin{frame}{Анализ исключением компонент и сравнение с нейросетями}
\justifying
Рассматривается датасет Haxby et al., состоящий из данных 6 испытуемых, объем $N=96$, число классов $C=8$. 
Тестовая выборка~-- 20\%. Метрики усреднены по 6 выборкам. 
Статистическая значимость обозначена *$p<.05$, **$p<.005$.
\begin{table}
    \centering
    \caption{Анализ исключением компонент. Рассматриваются 3 упрощенные модели.}\label{table:model_ablation}
    \resizebox{\textwidth}{!}{
...
\begin{table}
    \centering
    \caption{Сравнение с нейросетевыми архитектурами.}
    \vspace{-5mm}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{l ccc ccc}
        \toprule
...
\[
\mathfrak{D}=\{(\mathbf{F}_i,\mathbf{E}_i,\mathbf{I}_i)\mid i=1,\ldots,N\},\qquad (\mathbf{F}_i,\mathbf{E}_i)\in\mathcal{X}_{f}\times\mathcal{X}_{e},\qquad \mathbf{I}_i\in\mathcal{Y}.
\]
...
\begin{figure}
    \centering
    \includegraphics[width=0.55\linewidth]{figs/scheme.pdf}
\end{figure}
\end{frame}
%----------------------------------------------------------------------------------------------------------
...
\begin{enumerate}
\justifying
    \item Построен метод восстановления показаний фМРТ по видеоряду, просматриваемому человеком. Обоснован учет гемодинамической задержки для согласования стимула и BOLD-отклика.
    \item Разработан метод классификации временных рядов фМРТ в услових ограниченного объема выборки. Предложен алгоритм декодирования активности мозга. Проведено сравнение с нейросетевыми подходами.
...
\end{frame}
%----------------------------------------------------------------------------------------------------------
\begin{frame}{Список работ автора по теме доклада}
\footnotesize

\begin{block}{\small Публикации в журналах ВАК}
\begin{enumerate}
\justifying
...
\begin{block}{\small Выступления с докладом}
\begin{enumerate}
\justifying
    \item Пространственно-временные методы анализа временных рядов // 66-я Всероссийская научная конференция МФТИ, 2024.
    \item Улучшение декодирования фМРТ в условиях ограниченной выборки // 67-я Всероссийская научная конференция МФТИ, 2025.
    \item Декодирование визуальных стимулов из мультимодальных сигналов мозга // ММРО-2025.
    \item Улучшение декодирования данных фМРТ в условиях ограниченного набора данных // AI Journey 2025.
\end{enumerate}
\end{block}
\end{frame}
\end{document}